...
class Phone2Char_CTC_GAN_Solver(Phone2Char_CTC_Solver):
    ...
    def __init__(self, model, config, tr_loader, cv_loader, tokenizer):
        acoustic_loader, text_loader, tr_loader = tr_loader
        self.tr_loader = tr_loader
        self.acoustic_loader = acoustic_loader
        self.text_loader = text_loader
        super().__init__(model, config, tr_loader, cv_loader, tokenizer)

    def iter_semi_train_epoch(self):
        tr_loader = self.tr_loader
        acoustic_loader = self.acoustic_loader
        ...
        tot_iter_num = len(acoustic_loader)
        for niter, (phones, text, (utts, data)) in enumerate(zip(acoustic_loader, text_loader, tr_loader)):
            niter += 1

            if n_accu_batch == self.accumulate_grad_batch:
                self.optimizer.zero_grad()

            x, len_x = (i.to(self.device) for i in phones)
            text, len_text = (i.to(self.device) for i in text)
            _x, _len_x, _, _y, _y_paddings = (i.to(self.device) for i in data)

            # supervise loss
            ctc_loss = self.model(_x, _len_x, _y, _y_paddings)
            n_token = torch.sum(1-_y_paddings).float()
            tot_token += n_token
            n_sequence = len(utts)
            tot_sequence += n_sequence
            tot_loss += ctc_loss
            ...
            self.model.D.eval()
            self.model.D.freeze()
            G_loss = self.model.forward_G(x, len_x)
            G_loss.backward()
            self.model.D.train()
            self.model.D.unfreeze()

            # D update
            self.model.G.eval()
            D_loss = self.model.forward_D(x, len_x, text, len_text)
            D_loss.backward()
            self.model.G.train()
            ...
            timer.toc()
            if niter % self.print_inteval == 0:
                print('Epoch {} | Step {} | Iter {} batch {} {} \nctc_loss: {:.3f} G_loss: {:.3f} D_loss: {:.3f} lr: {:.3e} sent/sec: {:.3f}\n'.format(
                    self.epoch, self.step, niter, list(x.size()), list(_x.size()),
                    loss, G_loss, D_loss, list(self.optimizer.param_groups)[0]["lr"], tot_sequence/timer.toc()
                ), flush=True)
    ...
    def train(self):
        ...
        while self.epoch < self.num_epoch:
            ...
            self.epoch += 1
            logging.info("Training")
            tr_loss = self.iter_semi_train_epoch()
            tr_msg = ("tr loss: {:.4f}").format(tr_loss)
            msg = "\n" + "-"*85 + "\n"
            msg += "Epoch {} Training Summary:\n{}\n".format(self.epoch, tr_msg)
            msg += "-"*85
